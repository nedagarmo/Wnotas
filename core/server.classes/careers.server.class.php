...
switch($action){
	case "create":
		$result = $dao->insert_career($sigla, $nombre, $descripcion);
		if($result){
			$jtable_result['Result'] = "OK";
			$jtable_result['Record'] = $dao->get_career($sigla)->fields;
		} else {
		...
		break;
	case "update":
		$result = $dao->update_career($id, $sigla, $nombre, $descripcion);
		if($result){
			$jtable_result['Result'] = "OK";
		} else {
			$jtable_result['Result'] = "ERROR";
	...
	case "search":
		if(empty($name)){
			$result = $dao->get_all_careers($jtSorting, $jtStartIndex, $jtPageSize);
		}else{
			$result = $dao->get_career_search($name, $jtSorting, $jtStartIndex, $jtPageSize);
		}
		...
		if($result != null){
			...
			$jtable_result['TotalRecordCount'] = $dao->get_all_count_careers()->fields[0];	
		}else{
			$jtable_result['Result'] = "ERROR";
