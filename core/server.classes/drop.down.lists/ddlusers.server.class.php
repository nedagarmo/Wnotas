<?php

/**
 * Clase de servidor que devuelve una lista de datos para alimentar un control dropdownlist
 *
 * @author Nelson D. Garzón M.
 */

session_start();
extract($_REQUEST);
include_once(dirname(__FILE__) . '/../../data.access.objects/users.dao.class.php');

$dao = new users();
if($_SESSION['role'] == 1){
	if(isset($registered)){
		if(isset($students)){
			$result = $dao->get_all_users_list_by_registered_only_students();
		} else {
			$result = $dao->get_all_users_list_by_registered();
		}
	}else{
		$result = $dao->get_all_users_list();
	}
}else if($_SESSION['role'] == 2){
	$result = $dao->get_all_users_list_by_registered_by_teacher_only_students($_SESSION['id']);
} else {
	$result = $dao->get_all_users_list_by_registered_by_student_only_students($_SESSION['id']);
}

if($result != null){
	$rows = array();
	$counter = 0;
	while (!$result->EOF){			
		$rows[$counter]['DisplayText'] = $result->fields[2]." - ".$result->fields[1];
		$rows[$counter]['Value'] = $result->fields[0] + 0;
		$counter ++;
		$result->MoveNext();
	}
	$jtable_result['Result'] = "OK";
	$jtable_result['Options'] = $rows;
}else{
	$jtable_result['Result'] = "ERROR";
	$jtable_result['Message'] = "No se encontraron datos en el sistema.";
}

print json_encode($jtable_result);
?>
