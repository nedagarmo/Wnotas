    ...
    init: {
      ...
      desktop: function() {
        ...
        d.on('contextmenu', function(e) {
			$("#contextual_menu").css({'left':e.pageX, 'top':e.pageY - 20}).toggle('fade',1500);
			return false;
        });
      ...
      wallpaper: function() {
        ...
        if ($('#desktop').length) {
			$.post( "../core/server.classes/personalizations.server.class.php?action=background", function(data) {
				$('body').prepend('<img id="wallpaper" class="abs" src="../images/backgrounds/'+data+'" />');
			});
        }
      }
    },
...
$(document).ready(function(){            
	...
	$("#contextual_menu").click(function(e){
		  // El switch utiliza los IDs de los <li> del menú
		  switch(e.target.id){
				case "configurations": 
					var x = "#icon_dock_configurations";
					break;
				case "myprofile":
					var x = "#icon_dock_myprofile";
					break;
				case "personalization":
					var x = "#icon_dock_personalization";
					break;
		  }
		  
		  // Get the link's target.
		  var salida = '';
		  for (var p in e.target) {
			salida += p + ': ' + e.target[p] + '\n';
		  }
		  //alert(salida);
		  var y = $(x).find('a').attr('href');
